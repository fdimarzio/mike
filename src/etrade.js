...
// ── Low-level fetch (browser → Vite proxy → E*TRADE sandbox) ─────────────────
...
export async function fetchQuotes(tickers) {
  ...
  await Promise.all(
    chunks.map(async (chunk) => {
      ...
      const data = await etradeGet(`/v1/market/quote/${symbols}`);
      // Response: { QuoteResponse: { QuoteData: [...] } }
      const list = data?.QuoteResponse?.QuoteData ?? [];
      for (const q of list) {
        const sym = q?.Product?.symbol;
        const all = q?.All ?? q?.Intraday ?? {};
        if (sym) {
          results[sym.toUpperCase()] = {
            lastPrice:   all.lastTrade   ?? all.lastPrice ?? null,
            bid:         all.bid         ?? null,
            ask:         all.ask         ?? null,
            changeClose: all.changeClose ?? null,
            changePct:   all.changeClosePercentage ?? null,
            volume:      all.totalVolume ?? null,
...
export async function fetchOptionChain(ticker, expiryDate) {
  ...
  const data = await etradeGet("/v1/market/optionchains", {
    symbol:         ticker.toUpperCase(),
    expiryYear:     year,
    expiryMonth:    month,
    expiryDay:      day,
    optionCategory: "STANDARD",  // STANDARD | ALL | MINI
    chainType:      "CALLPUT",
    priceType:      "DEFAULT",   // DEFAULT | NBBO
    noOfStrikes:    30,          // strikes each side of ATM
  });
